...
CustomerAPI_createCustomer(apiClient_t *apiClient, customer_create_t *customer_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/customers");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_customer_create = NULL;
    if (customer_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_customer_create = customer_create_convertToJSON(customer_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_customer_create);
    ...
    customer_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CustomerAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = customer_parseFromJSON(CustomerAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_customer_create) {
        cJSON_Delete(localVarSingleItemJSON_customer_create);
        localVarSingleItemJSON_customer_create = NULL;
    }
...
CustomerAPI_customerRestore(apiClient_t *apiClient, char *customer_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/customers/{customer_id}/restore");
...
CustomerAPI_deleteCustomer(apiClient_t *apiClient, char *customer_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/customers/{customer_id}");
...
CustomerAPI_getCustomers(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CustomerAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(CustomerAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, CustomerAPIlocalVarJSON)
        {
