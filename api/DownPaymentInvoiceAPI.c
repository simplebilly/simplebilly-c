...
#include "DownPaymentInvoiceAPI.h"
...
DownPaymentInvoiceAPI_downloadDownPaymentInvoicePdf(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/down-payment-invoices/{id}/pdf");
...
DownPaymentInvoiceAPI_getDownPaymentInvoice(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/down-payment-invoices/{id}");
    ...
    down_payment_invoice_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *DownPaymentInvoiceAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = down_payment_invoice_parseFromJSON(DownPaymentInvoiceAPIlocalVarJSON);
        cJSON_Delete(DownPaymentInvoiceAPIlocalVarJSON);
...
DownPaymentInvoiceAPI_listDownPaymentInvoices(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/down-payment-invoices/");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *DownPaymentInvoiceAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(DownPaymentInvoiceAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, DownPaymentInvoiceAPIlocalVarJSON)
        {
