...
GewinnverwendungAPI_gewinnverwendungApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/gewinnverwendung");
    ...
    //    printf("%s\n","Gewinnverwendungsrechnung (§ 150, § 174 AktG)");
    //}
    //nonprimitive not container
    gewinnverwendungs_report_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GewinnverwendungAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = gewinnverwendungs_report_parseFromJSON(GewinnverwendungAPIlocalVarJSON);
        cJSON_Delete(GewinnverwendungAPIlocalVarJSON);
...
GewinnverwendungAPI_gewinnverwendungExportApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/gewinnverwendung/export");
    ...
    // uncomment below to debug the error response
    //if (apiClient->response_code == 200) {
    //    printf("%s\n","Gewinnverwendungsrechnung als CSV (BOM, Semikolon)");
    //}
    //nonprimitive not container
    gewinnverwendungs_export_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GewinnverwendungAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = gewinnverwendungs_export_response_parseFromJSON(GewinnverwendungAPIlocalVarJSON);
        cJSON_Delete(GewinnverwendungAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
