...
QuotationAPI_createQuotation(apiClient_t *apiClient, quotation_create_t *quotation_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/quotations");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_quotation_create = NULL;
    if (quotation_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_quotation_create = quotation_create_convertToJSON(quotation_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_quotation_create);
    ...
    quotation_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *QuotationAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = quotation_parseFromJSON(QuotationAPIlocalVarJSON);
        cJSON_Delete(QuotationAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_quotation_create) {
        cJSON_Delete(localVarSingleItemJSON_quotation_create);
        localVarSingleItemJSON_quotation_create = NULL;
    }
...
QuotationAPI_deleteQuotation(apiClient_t *apiClient, char *quotation_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/quotations/{quotation_id}");
    ...
    // Path Params
    long sizeOfPathParams_quotation_id = strlen(quotation_id)+3 + sizeof("{ quotation_id }") - 1;
    if(quotation_id == NULL) {
        goto end;
    }
    char* localVarToReplace_quotation_id = malloc(sizeOfPathParams_quotation_id);
    sprintf(localVarToReplace_quotation_id, "{%s}", "quotation_id");

    localVarPath = strReplace(localVarPath, localVarToReplace_quotation_id, quotation_id);
    ...
    free(localVarPath);
    free(localVarToReplace_quotation_id);
...
QuotationAPI_downloadQuotationPdf(apiClient_t *apiClient, char *quotation_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/quotations/{quotation_id}/pdf");
...
QuotationAPI_listQuotations(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *QuotationAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(QuotationAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, QuotationAPIlocalVarJSON)
        {
...
QuotationAPI_pursueQuotation(apiClient_t *apiClient, char *quotation_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/quotations/{quotation_id}/pursue");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *QuotationAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = order_confirmation_parseFromJSON(QuotationAPIlocalVarJSON);
        cJSON_Delete(QuotationAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
...
QuotationAPI_quotationRestore(apiClient_t *apiClient, char *quotation_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/quotations/{quotation_id}/restore");
