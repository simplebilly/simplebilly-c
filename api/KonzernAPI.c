...
KonzernAPI_konzernExportApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/konzern/status/export");
    ...
    //    printf("%s\n","Konzern-Beteiligungen mit Kontrollstatus als CSV (BOM, Semikolon)");
    //}
    //nonprimitive not container
    konzern_export_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *KonzernAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = konzern_export_response_parseFromJSON(KonzernAPIlocalVarJSON);
        cJSON_Delete(KonzernAPIlocalVarJSON);
...
KonzernAPI_konzernStatusApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/konzern/status");
    ...
    // uncomment below to debug the error response
    //if (apiClient->response_code == 200) {
    //    printf("%s\n","Konzern-Status (§§ 290, 291, 293 HGB)");
    //}
    //nonprimitive not container
    konzern_status_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *KonzernAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = konzern_status_parseFromJSON(KonzernAPIlocalVarJSON);
        cJSON_Delete(KonzernAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
