...
PaymentAPI_createPayment(apiClient_t *apiClient, payment_create_t *payment_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/payments");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_payment_create = NULL;
    if (payment_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_payment_create = payment_create_convertToJSON(payment_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_payment_create);
    ...
    payment_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *PaymentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = payment_parseFromJSON(PaymentAPIlocalVarJSON);
        cJSON_Delete(PaymentAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_payment_create) {
        cJSON_Delete(localVarSingleItemJSON_payment_create);
        localVarSingleItemJSON_payment_create = NULL;
    }
...
PaymentAPI_deletePayment(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/payments/{id}");
...
PaymentAPI_getPayments(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *PaymentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(PaymentAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, PaymentAPIlocalVarJSON)
        {
...
PaymentAPI_paymentRestore(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/payments/{id}/restore");
