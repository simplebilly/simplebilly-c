...
#include "RecurringTemplateAPI.h"
...
RecurringTemplateAPI_createRecurringTemplate(apiClient_t *apiClient, body)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/recurring-templates");
    ...
    recurring_template_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *RecurringTemplateAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = recurring_template_parseFromJSON(RecurringTemplateAPIlocalVarJSON);
        cJSON_Delete(RecurringTemplateAPIlocalVarJSON);
...
RecurringTemplateAPI_deleteRecurringTemplate(apiClient_t *apiClient, char *template_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/recurring-templates/{template_id}");
    ...
    // Path Params
    long sizeOfPathParams_template_id = strlen(template_id)+3 + sizeof("{ template_id }") - 1;
    if(template_id == NULL) {
        goto end;
    }
...
RecurringTemplateAPI_listRecurringTemplates(apiClient_t *apiClient)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *RecurringTemplateAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(RecurringTemplateAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, RecurringTemplateAPIlocalVarJSON)
        {
