...
OrderAPI_addOrderTags(apiClient_t *apiClient, char *order_id, order_tags_request_t *order_tags_request)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/orders/{order_id}/tags");
    ...
    // Path Params
    long sizeOfPathParams_order_id = strlen(order_id)+3 + sizeof("{ order_id }") - 1;
    if(order_id == NULL) {
        goto end;
    }
    char* localVarToReplace_order_id = malloc(sizeOfPathParams_order_id);
    sprintf(localVarToReplace_order_id, "{%s}", "order_id");

    localVarPath = strReplace(localVarPath, localVarToReplace_order_id, order_id);



    // Body Param
    cJSON *localVarSingleItemJSON_order_tags_request = NULL;
    if (order_tags_request != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_order_tags_request = order_tags_request_convertToJSON(order_tags_request);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_order_tags_request);
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        ...
        elementToReturn = order_parseFromJSON(OrderAPIlocalVarJSON);
    ...
    free(localVarPath);
    free(localVarToReplace_order_id);
    if (localVarSingleItemJSON_order_tags_request) {
        cJSON_Delete(localVarSingleItemJSON_order_tags_request);
        localVarSingleItemJSON_order_tags_request = NULL;
    }
...
OrderAPI_findOrderByExternalRef(apiClient_t *apiClient, char *ext_ref)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/orders/by-ext-ref/{ext_ref}");
    ...
    // Path Params
    long sizeOfPathParams_ext_ref = strlen(ext_ref)+3 + sizeof("{ ext_ref }") - 1;
    if(ext_ref == NULL) {
        goto end;
    }
    char* localVarToReplace_ext_ref = malloc(sizeOfPathParams_ext_ref);
    sprintf(localVarToReplace_ext_ref, "{%s}", "ext_ref");

    localVarPath = strReplace(localVarPath, localVarToReplace_ext_ref, ext_ref);
    ...
    free(localVarPath);
    free(localVarToReplace_ext_ref);
    return elementToReturn;
end:
    free(localVarPath);
    return NULL;
...
OrderAPI_getOrder(apiClient_t *apiClient, char *order_number)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/order/{order_number}");
...
OrderAPI_getOrders(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/orders");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *OrderAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(OrderAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, OrderAPIlocalVarJSON)
        {
...
OrderAPI_patchOrder(apiClient_t *apiClient, char *order_id, body)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/orders/{order_id}");
...
OrderAPI_updateOrderState(apiClient_t *apiClient, char *order_id, order_state_update_t *order_state_update)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/orders/{order_id}/state");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_order_state_update = NULL;
    if (order_state_update != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_order_state_update = order_state_update_convertToJSON(order_state_update);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_order_state_update);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    free(localVarPath);
    free(localVarToReplace_order_id);
    if (localVarSingleItemJSON_order_state_update) {
        cJSON_Delete(localVarSingleItemJSON_order_state_update);
        localVarSingleItemJSON_order_state_update = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
