...
EuerAPI_euerApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/euer");
    ...
    euer_ergebnis_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *EuerAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = euer_ergebnis_parseFromJSON(EuerAPIlocalVarJSON);
        cJSON_Delete(EuerAPIlocalVarJSON);
...
EuerAPI_euerKategorienApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/euer/kategorien");
    ...
    // uncomment below to debug the error response
    //if (apiClient->response_code == 200) {
    //    printf("%s\n","EÜR mit Kategorie-Aufschlüsselung");
    //}
    //nonprimitive not container
    euer_detail_ergebnis_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *EuerAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = euer_detail_ergebnis_parseFromJSON(EuerAPIlocalVarJSON);
        cJSON_Delete(EuerAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
