...
#include "GenerateXrechnungAPI.h"
...
GenerateXrechnungAPI_generateXrechnungApi(apiClient_t *apiClient, char *id, char *supplier_name, char *supplier_street, char *supplier_city, char *supplier_zip, char *supplier_country, char *supplier_vat_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/invoices/{id}/xrechnung");
    ...
    x_rechnung_response_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GenerateXrechnungAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = x_rechnung_response_parseFromJSON(GenerateXrechnungAPIlocalVarJSON);
        cJSON_Delete(GenerateXrechnungAPIlocalVarJSON);
