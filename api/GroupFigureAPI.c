...
GroupFigureAPI_createGroupFigure(apiClient_t *apiClient, group_figure_create_t *group_figure_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/group-figures");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_group_figure_create = NULL;
    if (group_figure_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_group_figure_create = group_figure_create_convertToJSON(group_figure_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_group_figure_create);
    ...
    group_figure_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GroupFigureAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = group_figure_parseFromJSON(GroupFigureAPIlocalVarJSON);
        cJSON_Delete(GroupFigureAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_group_figure_create) {
        cJSON_Delete(localVarSingleItemJSON_group_figure_create);
        localVarSingleItemJSON_group_figure_create = NULL;
    }
...
GroupFigureAPI_deleteGroupFigure(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/group-figures/{year}");



    // Path Params
    long sizeOfPathParams_year =  + sizeof("{ year }") - 1;
    if(year == 0){
        goto end;
    }
    char* localVarToReplace_year = malloc(sizeOfPathParams_year);
    snprintf(localVarToReplace_year, sizeOfPathParams_year, "{%s}", "year");

    char localVarBuff_year[256];
    snprintf(localVarBuff_year, sizeof localVarBuff_year, "%ld", (long)*year);

    localVarPath = strReplace(localVarPath, localVarToReplace_year, localVarBuff_year);
...
GroupFigureAPI_getGroupFigures(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GroupFigureAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(GroupFigureAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, GroupFigureAPIlocalVarJSON)
        {
...
GroupFigureAPI_updateGroupFigure(apiClient_t *apiClient, int *year, group_figure_update_t *group_figure_update)
{
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_group_figure_update = NULL;
    if (group_figure_update != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_group_figure_update = group_figure_update_convertToJSON(group_figure_update);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_group_figure_update);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    free(localVarPath);
    free(localVarToReplace_year);
    if (localVarSingleItemJSON_group_figure_update) {
        cJSON_Delete(localVarSingleItemJSON_group_figure_update);
        localVarSingleItemJSON_group_figure_update = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
