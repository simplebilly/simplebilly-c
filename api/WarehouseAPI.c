...
WarehouseAPI_createWarehouse(apiClient_t *apiClient, warehouse_t *warehouse)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/warehouses");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_warehouse = NULL;
    if (warehouse != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_warehouse = warehouse_convertToJSON(warehouse);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_warehouse);
    ...
    warehouse_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *WarehouseAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = warehouse_parseFromJSON(WarehouseAPIlocalVarJSON);
        cJSON_Delete(WarehouseAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_warehouse) {
        cJSON_Delete(localVarSingleItemJSON_warehouse);
        localVarSingleItemJSON_warehouse = NULL;
    }
...
WarehouseAPI_deleteWarehouse(apiClient_t *apiClient, char *warehouse_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/warehouses/{warehouse_id}");
...
WarehouseAPI_listWarehouses(apiClient_t *apiClient, int *page, int *page_size, char *search, int *is_active)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *WarehouseAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(WarehouseAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, WarehouseAPIlocalVarJSON)
        {
