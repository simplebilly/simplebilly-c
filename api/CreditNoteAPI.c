...
CreditNoteAPI_createCreditNote(apiClient_t *apiClient, body)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/credit-notes");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CreditNoteAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = invoice_parseFromJSON(CreditNoteAPIlocalVarJSON);
        cJSON_Delete(CreditNoteAPIlocalVarJSON);
...
CreditNoteAPI_downloadCreditNotePdf(apiClient_t *apiClient, char *credit_note_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/credit-notes/{credit_note_id}/pdf");
    ...
    // Path Params
    long sizeOfPathParams_credit_note_id = strlen(credit_note_id)+3 + sizeof("{ credit_note_id }") - 1;
    if(credit_note_id == NULL) {
        goto end;
    }
    char* localVarToReplace_credit_note_id = malloc(sizeOfPathParams_credit_note_id);
    sprintf(localVarToReplace_credit_note_id, "{%s}", "credit_note_id");

    localVarPath = strReplace(localVarPath, localVarToReplace_credit_note_id, credit_note_id);
    ...
    free(localVarPath);
    free(localVarToReplace_credit_note_id);
...
CreditNoteAPI_getCreditNote(apiClient_t *apiClient, char *credit_note_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/credit-notes/{credit_note_id}");
...
CreditNoteAPI_listCreditNotes(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *CreditNoteAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(CreditNoteAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, CreditNoteAPIlocalVarJSON)
        {
