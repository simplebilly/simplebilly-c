...
#include "TrainingAssignmentAPI.h"
...
TrainingAssignmentAPI_createTrainingAssignment(apiClient_t *apiClient, training_assignment_create_t *training_assignment_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/training-assignments");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_training_assignment_create = NULL;
    if (training_assignment_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_training_assignment_create = training_assignment_create_convertToJSON(training_assignment_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_training_assignment_create);
    ...
    training_assignment_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *TrainingAssignmentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = training_assignment_parseFromJSON(TrainingAssignmentAPIlocalVarJSON);
        cJSON_Delete(TrainingAssignmentAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_training_assignment_create) {
        cJSON_Delete(localVarSingleItemJSON_training_assignment_create);
        localVarSingleItemJSON_training_assignment_create = NULL;
    }
...
TrainingAssignmentAPI_deleteTrainingAssignment(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/training-assignments/{id}");
...
TrainingAssignmentAPI_getTrainingAssignments(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *TrainingAssignmentAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(TrainingAssignmentAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, TrainingAssignmentAPIlocalVarJSON)
        {
...
TrainingAssignmentAPI_updateTrainingAssignment(apiClient_t *apiClient, char *id, training_assignment_update_t *training_assignment_update)
{
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_training_assignment_update = NULL;
    if (training_assignment_update != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_training_assignment_update = training_assignment_update_convertToJSON(training_assignment_update);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_training_assignment_update);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    free(localVarPath);
    free(localVarToReplace_id);
    if (localVarSingleItemJSON_training_assignment_update) {
        cJSON_Delete(localVarSingleItemJSON_training_assignment_update);
        localVarSingleItemJSON_training_assignment_update = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
