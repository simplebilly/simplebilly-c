...
AnlageEksAPI_eksApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/eks");
    ...
    eks_ergebnis_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AnlageEksAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = eks_ergebnis_parseFromJSON(AnlageEksAPIlocalVarJSON);
        cJSON_Delete(AnlageEksAPIlocalVarJSON);
