...
AbsenceAPI_createAbsence(apiClient_t *apiClient, absence_create_t *absence_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/absences");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_absence_create = NULL;
    if (absence_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_absence_create = absence_create_convertToJSON(absence_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_absence_create);
    ...
    absence_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AbsenceAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = absence_parseFromJSON(AbsenceAPIlocalVarJSON);
        cJSON_Delete(AbsenceAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_absence_create) {
        cJSON_Delete(localVarSingleItemJSON_absence_create);
        localVarSingleItemJSON_absence_create = NULL;
    }
...
AbsenceAPI_deleteAbsence(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/absences/{id}");
...
AbsenceAPI_getAbsences(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AbsenceAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(AbsenceAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, AbsenceAPIlocalVarJSON)
        {
...
AbsenceAPI_updateAbsence(apiClient_t *apiClient, char *id, absence_update_t *absence_update)
{
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_absence_update = NULL;
    if (absence_update != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_absence_update = absence_update_convertToJSON(absence_update);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_absence_update);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    free(localVarPath);
    free(localVarToReplace_id);
    if (localVarSingleItemJSON_absence_update) {
        cJSON_Delete(localVarSingleItemJSON_absence_update);
        localVarSingleItemJSON_absence_update = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
