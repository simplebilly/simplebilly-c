...
AnlageSAPI_anlageSApi(apiClient_t *apiClient, int *year)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/bookkeeping/anlage-s");
    ...
    anlage_s_ergebnis_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *AnlageSAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = anlage_s_ergebnis_parseFromJSON(AnlageSAPIlocalVarJSON);
        cJSON_Delete(AnlageSAPIlocalVarJSON);
