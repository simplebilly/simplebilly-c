...
ShareholderAPI_createShareholder(apiClient_t *apiClient, shareholder_create_t *shareholder_create)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/shareholders");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_shareholder_create = NULL;
    if (shareholder_create != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_shareholder_create = shareholder_create_convertToJSON(shareholder_create);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_shareholder_create);
    ...
    shareholder_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *ShareholderAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = shareholder_parseFromJSON(ShareholderAPIlocalVarJSON);
        cJSON_Delete(ShareholderAPIlocalVarJSON);
    ...
    free(localVarPath);
    if (localVarSingleItemJSON_shareholder_create) {
        cJSON_Delete(localVarSingleItemJSON_shareholder_create);
        localVarSingleItemJSON_shareholder_create = NULL;
    }
...
ShareholderAPI_deleteShareholder(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v1/shareholders/{id}");
...
ShareholderAPI_getShareholders(apiClient_t *apiClient, int *page, int *page_size, char *search, int *include_deleted)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *ShareholderAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(ShareholderAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, ShareholderAPIlocalVarJSON)
        {
...
ShareholderAPI_updateShareholder(apiClient_t *apiClient, char *id, shareholder_update_t *shareholder_update)
{
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_shareholder_update = NULL;
    if (shareholder_update != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_shareholder_update = shareholder_update_convertToJSON(shareholder_update);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_shareholder_update);
        localVarBodyLength = strlen(localVarBodyParameters);
    }
    list_addElement(localVarHeaderType,"application/json"); //produces
    list_addElement(localVarContentType,"application/json"); //consumes
    ...
    free(localVarPath);
    free(localVarToReplace_id);
    if (localVarSingleItemJSON_shareholder_update) {
        cJSON_Delete(localVarSingleItemJSON_shareholder_update);
        localVarSingleItemJSON_shareholder_update = NULL;
    }
    free(localVarBodyParameters);
    return elementToReturn;
end:
