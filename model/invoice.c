...
static invoice_t *invoice_create_internal(
    ...
    invoice_local_var->attachments = attachments;
    invoice_local_var->billing_period_end = billing_period_end;
    invoice_local_var->billing_period_start = billing_period_start;
    invoice_local_var->cancellation_date = cancellation_date;
    invoice_local_var->cancellation_invoice_id = cancellation_invoice_id;
    invoice_local_var->cancellation_reason = cancellation_reason;
    invoice_local_var->contract_id = contract_id;
    invoice_local_var->currency = currency;
    invoice_local_var->customer_id = customer_id;
    invoice_local_var->discount_amount = discount_amount;
    invoice_local_var->discount_days = discount_days;
    invoice_local_var->discount_percentage = discount_percentage;
    invoice_local_var->document_type = document_type;
    invoice_local_var->dunning_level = dunning_level;
    invoice_local_var->input_vat_amount = input_vat_amount;
    invoice_local_var->input_vat_deductible = input_vat_deductible;
    invoice_local_var->input_vat_percentage = input_vat_percentage;
    invoice_local_var->introduction_text = introduction_text;
    invoice_local_var->invoice_type = invoice_type;
    invoice_local_var->is_cancelled = is_cancelled;
    invoice_local_var->is_draft = is_draft;
    invoice_local_var->is_eu_acquisition = is_eu_acquisition;
    invoice_local_var->is_eu_delivery = is_eu_delivery;
    invoice_local_var->is_intra_community_acquisition = is_intra_community_acquisition;
    invoice_local_var->is_reverse_charge = is_reverse_charge;
    invoice_local_var->issue_date = issue_date;
    invoice_local_var->ledger_account = ledger_account;
    invoice_local_var->line_items = line_items;
    invoice_local_var->margin25a = margin25a;
    invoice_local_var->margin25a_gross = margin25a_gross;
    invoice_local_var->margin25a_purchase_price = margin25a_purchase_price;
    invoice_local_var->notes = notes;
    invoice_local_var->order_number = order_number;
    invoice_local_var->original_pdf_path = original_pdf_path;
    invoice_local_var->paid_amount = paid_amount;
    invoice_local_var->payment_due_date = payment_due_date;
    invoice_local_var->payment_status = payment_status;
    invoice_local_var->payment_terms_text = payment_terms_text;
    invoice_local_var->preceding_sales_voucher_id = preceding_sales_voucher_id;
    invoice_local_var->preceding_sales_voucher_type = preceding_sales_voucher_type;
    invoice_local_var->receipt_confirmation_available = receipt_confirmation_available;
    invoice_local_var->related_invoice_id = related_invoice_id;
    invoice_local_var->relationship_type = relationship_type;
    invoice_local_var->sender_snapshot = sender_snapshot;
    invoice_local_var->sent_at = sent_at;
    invoice_local_var->service_period_end = service_period_end;
    invoice_local_var->service_period_start = service_period_start;
    invoice_local_var->status = status;
    invoice_local_var->subtotal = subtotal;
    invoice_local_var->supplier_id = supplier_id;
    invoice_local_var->tax_exemption_reason = tax_exemption_reason;
    invoice_local_var->total_amount = total_amount;
    invoice_local_var->total_tax = total_tax;
    invoice_local_var->vat_country = vat_country;
    invoice_local_var->vat_special_case = vat_special_case;
...
void invoice_free(invoice_t *invoice) {
    ...
    if (invoice->attachments) {
        _free(invoice->attachments);
        invoice->attachments = NULL;
    }
    if (invoice->billing_period_end) {
        free(invoice->billing_period_end);
        invoice->billing_period_end = NULL;
    }
    if (invoice->billing_period_start) {
        free(invoice->billing_period_start);
        invoice->billing_period_start = NULL;
    }
    if (invoice->cancellation_date) {
        free(invoice->cancellation_date);
        invoice->cancellation_date = NULL;
    }
    if (invoice->cancellation_invoice_id) {
        free(invoice->cancellation_invoice_id);
        invoice->cancellation_invoice_id = NULL;
    }
    if (invoice->cancellation_reason) {
        free(invoice->cancellation_reason);
        invoice->cancellation_reason = NULL;
    }
    if (invoice->contract_id) {
        free(invoice->contract_id);
        invoice->contract_id = NULL;
    }
    if (invoice->customer_id) {
        free(invoice->customer_id);
        invoice->customer_id = NULL;
    }
    if (invoice->discount_amount) {
        free(invoice->discount_amount);
        invoice->discount_amount = NULL;
    }
    if (invoice->discount_days) {
        free(invoice->discount_days);
        invoice->discount_days = NULL;
    }
    if (invoice->discount_percentage) {
        free(invoice->discount_percentage);
        invoice->discount_percentage = NULL;
    }
    if (invoice->dunning_level) {
        free(invoice->dunning_level);
        invoice->dunning_level = NULL;
    }
    if (invoice->input_vat_amount) {
        free(invoice->input_vat_amount);
        invoice->input_vat_amount = NULL;
    }
    if (invoice->input_vat_deductible) {
        free(invoice->input_vat_deductible);
        invoice->input_vat_deductible = NULL;
    }
    if (invoice->input_vat_percentage) {
        free(invoice->input_vat_percentage);
        invoice->input_vat_percentage = NULL;
    }
    if (invoice->introduction_text) {
        free(invoice->introduction_text);
        invoice->introduction_text = NULL;
    }
    if (invoice->is_cancelled) {
        free(invoice->is_cancelled);
        invoice->is_cancelled = NULL;
    }
    if (invoice->is_draft) {
        free(invoice->is_draft);
        invoice->is_draft = NULL;
    }
    if (invoice->is_eu_acquisition) {
        free(invoice->is_eu_acquisition);
        invoice->is_eu_acquisition = NULL;
    }
    if (invoice->is_eu_delivery) {
        free(invoice->is_eu_delivery);
        invoice->is_eu_delivery = NULL;
    }
    if (invoice->is_intra_community_acquisition) {
        free(invoice->is_intra_community_acquisition);
        invoice->is_intra_community_acquisition = NULL;
    }
    if (invoice->is_reverse_charge) {
        free(invoice->is_reverse_charge);
        invoice->is_reverse_charge = NULL;
    }
    if (invoice->issue_date) {
        free(invoice->issue_date);
        invoice->issue_date = NULL;
    }
    if (invoice->ledger_account) {
        free(invoice->ledger_account);
        invoice->ledger_account = NULL;
    }
    if (invoice->line_items) {
        _free(invoice->line_items);
        invoice->line_items = NULL;
    }
    if (invoice->margin25a) {
        free(invoice->margin25a);
        invoice->margin25a = NULL;
    }
    if (invoice->margin25a_gross) {
        free(invoice->margin25a_gross);
        invoice->margin25a_gross = NULL;
    }
    if (invoice->margin25a_purchase_price) {
        free(invoice->margin25a_purchase_price);
        invoice->margin25a_purchase_price = NULL;
    }
    if (invoice->notes) {
        free(invoice->notes);
        invoice->notes = NULL;
    ...
    if (invoice->order_number) {
        free(invoice->order_number);
        invoice->order_number = NULL;
    }
    if (invoice->original_pdf_path) {
        free(invoice->original_pdf_path);
        invoice->original_pdf_path = NULL;
    }
    if (invoice->paid_amount) {
        free(invoice->paid_amount);
        invoice->paid_amount = NULL;
    }
    if (invoice->payment_due_date) {
        free(invoice->payment_due_date);
        invoice->payment_due_date = NULL;
    }
    if (invoice->payment_terms_text) {
        free(invoice->payment_terms_text);
        invoice->payment_terms_text = NULL;
    }
    if (invoice->preceding_sales_voucher_id) {
        free(invoice->preceding_sales_voucher_id);
        invoice->preceding_sales_voucher_id = NULL;
    }
    if (invoice->preceding_sales_voucher_type) {
        preceding_sales_voucher_type_free(invoice->preceding_sales_voucher_type);
        invoice->preceding_sales_voucher_type = NULL;
    }
    if (invoice->receipt_confirmation_available) {
        free(invoice->receipt_confirmation_available);
        invoice->receipt_confirmation_available = NULL;
    }
    if (invoice->related_invoice_id) {
        free(invoice->related_invoice_id);
        invoice->related_invoice_id = NULL;
    }
    if (invoice->relationship_type) {
        free(invoice->relationship_type);
        invoice->relationship_type = NULL;
    }
    if (invoice->sender_snapshot) {
        _free(invoice->sender_snapshot);
        invoice->sender_snapshot = NULL;
    }
    if (invoice->sent_at) {
        free(invoice->sent_at);
        invoice->sent_at = NULL;
    }
    if (invoice->service_period_end) {
        free(invoice->service_period_end);
        invoice->service_period_end = NULL;
    }
    if (invoice->service_period_start) {
        free(invoice->service_period_start);
    ...
    if (invoice->supplier_id) {
        free(invoice->supplier_id);
        invoice->supplier_id = NULL;
    }
    if (invoice->tax_exemption_reason) {
        free(invoice->tax_exemption_reason);
        invoice->tax_exemption_reason = NULL;
    }
    if (invoice->total_amount) {
        free(invoice->total_amount);
        invoice->total_amount = NULL;
    }
    if (invoice->total_tax) {
        free(invoice->total_tax);
        invoice->total_tax = NULL;
    }
    if (invoice->vat_country) {
        country_code_free(invoice->vat_country);
        invoice->vat_country = NULL;
    }
    if (invoice->vat_special_case) {
        free(invoice->vat_special_case);
        invoice->vat_special_case = NULL;
    }
    free(invoice);
}
...
cJSON *invoice_convertToJSON(invoice_t *invoice) {
    ...
    // invoice->attachments
    if(invoice->attachments) {
    cJSON *attachments_local_JSON = _convertToJSON(invoice->attachments);
    ...
    if(invoice->billing_period_end) {
    if(cJSON_AddStringToObject(item, "billingPeriodEnd", invoice->billing_period_end) == NULL) {
    goto fail; //Date
    }
    }
    ...
    if(invoice->billing_period_start) {
    if(cJSON_AddStringToObject(item, "billingPeriodStart", invoice->billing_period_start) == NULL) {
    goto fail; //Date
    }
    }
    ...
    if(invoice->cancellation_date) {
    if(cJSON_AddStringToObject(item, "cancellationDate", invoice->cancellation_date) == NULL) {
    goto fail; //Date
    }
    }


    // invoice->cancellation_invoice_id
    if(invoice->cancellation_invoice_id) {
    if(cJSON_AddStringToObject(item, "cancellationInvoiceId", invoice->cancellation_invoice_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->cancellation_reason) {
    if(cJSON_AddStringToObject(item, "cancellationReason", invoice->cancellation_reason) == NULL) {
    goto fail; //String
    }
    }
    ...
    // invoice->contract_id
    if(invoice->contract_id) {
    if(cJSON_AddStringToObject(item, "contractId", invoice->contract_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    // invoice->currency
    if (simplebilly_api_currency_code__NULL == invoice->currency) {
        goto fail;
    }
    cJSON *currency_local_JSON = currency_code_convertToJSON(invoice->currency);
    ...
    // invoice->customer_id
    if(invoice->customer_id) {
    if(cJSON_AddStringToObject(item, "customerId", invoice->customer_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->discount_amount) {
    if(cJSON_AddStringToObject(item, "discountAmount", invoice->discount_amount) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->discount_days) {
    if(cJSON_AddNumberToObject(item, "discountDays", *invoice->discount_days) == NULL) {
    goto fail; //Numeric
    }
    }
    ...
    if(invoice->discount_percentage) {
    if(cJSON_AddStringToObject(item, "discountPercentage", invoice->discount_percentage) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->document_type != simplebilly_api_document_type__NULL) {
    cJSON *document_type_local_JSON = document_type_convertToJSON(invoice->document_type);
    ...
    if(invoice->dunning_level) {
    if(cJSON_AddNumberToObject(item, "dunningLevel", *invoice->dunning_level) == NULL) {
    goto fail; //Numeric
    }
    }


    // invoice->input_vat_amount
    if(invoice->input_vat_amount) {
    if(cJSON_AddStringToObject(item, "inputVatAmount", invoice->input_vat_amount) == NULL) {
    goto fail; //String
    }
    }


    // invoice->input_vat_deductible
    if(invoice->input_vat_deductible) {
    if(cJSON_AddBoolToObject(item, "inputVatDeductible", *invoice->input_vat_deductible) == NULL) {
    goto fail; //Bool
    }
    }


    // invoice->input_vat_percentage
    if(invoice->input_vat_percentage) {
    if(cJSON_AddStringToObject(item, "inputVatPercentage", invoice->input_vat_percentage) == NULL) {
    goto fail; //String
    }
    }


    // invoice->introduction_text
    if(invoice->introduction_text) {
    if(cJSON_AddStringToObject(item, "introductionText", invoice->introduction_text) == NULL) {
    goto fail; //String
    }
    }
    ...
    if (simplebilly_api_invoice_type__NULL == invoice->invoice_type) {
        goto fail;
    }
    cJSON *invoice_type_local_JSON = invoice_type_convertToJSON(invoice->invoice_type);
    ...
    if(invoice->is_cancelled) {
    if(cJSON_AddBoolToObject(item, "isCancelled", *invoice->is_cancelled) == NULL) {
    goto fail; //Bool
    }
    }
    ...
    // invoice->is_draft
    if(invoice->is_draft) {
    if(cJSON_AddBoolToObject(item, "isDraft", *invoice->is_draft) == NULL) {
    goto fail; //Bool
    }
    }


    // invoice->is_eu_acquisition
    if(invoice->is_eu_acquisition) {
    if(cJSON_AddBoolToObject(item, "isEuAcquisition", *invoice->is_eu_acquisition) == NULL) {
    goto fail; //Bool
    }
    }


    // invoice->is_eu_delivery
    if(invoice->is_eu_delivery) {
    if(cJSON_AddBoolToObject(item, "isEuDelivery", *invoice->is_eu_delivery) == NULL) {
    goto fail; //Bool
    }
    }


    // invoice->is_intra_community_acquisition
    if(invoice->is_intra_community_acquisition) {
    if(cJSON_AddBoolToObject(item, "isIntraCommunityAcquisition", *invoice->is_intra_community_acquisition) == NULL) {
    goto fail; //Bool
    }
    }
    ...
    if(invoice->is_reverse_charge) {
    if(cJSON_AddBoolToObject(item, "isReverseCharge", *invoice->is_reverse_charge) == NULL) {
    goto fail; //Bool
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "issueDate", invoice->issue_date) == NULL) {
    goto fail; //Date
    }


    // invoice->ledger_account
    if(invoice->ledger_account) {
    if(cJSON_AddStringToObject(item, "ledgerAccount", invoice->ledger_account) == NULL) {
    goto fail; //String
    }
    }
    ...
    cJSON *line_items_local_JSON = _convertToJSON(invoice->line_items);
    ...
    // invoice->margin25a
    if(invoice->margin25a) {
    if(cJSON_AddBoolToObject(item, "margin25a", *invoice->margin25a) == NULL) {
    goto fail; //Bool
    }
    }


    // invoice->margin25a_gross
    if(invoice->margin25a_gross) {
    if(cJSON_AddStringToObject(item, "margin25aGross", invoice->margin25a_gross) == NULL) {
    goto fail; //String
    }
    }


    // invoice->margin25a_purchase_price
    if(invoice->margin25a_purchase_price) {
    if(cJSON_AddStringToObject(item, "margin25aPurchasePrice", invoice->margin25a_purchase_price) == NULL) {
    goto fail; //String
    }
    }
    ...
    // invoice->notes
    if(invoice->notes) {
    if(cJSON_AddStringToObject(item, "notes", invoice->notes) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->order_number) {
    if(cJSON_AddStringToObject(item, "orderNumber", invoice->order_number) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->original_pdf_path) {
    if(cJSON_AddStringToObject(item, "originalPdfPath", invoice->original_pdf_path) == NULL) {
    goto fail; //String
    }
    }
    ...
    // invoice->paid_amount
    if(invoice->paid_amount) {
    if(cJSON_AddStringToObject(item, "paidAmount", invoice->paid_amount) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->payment_due_date) {
    if(cJSON_AddStringToObject(item, "paymentDueDate", invoice->payment_due_date) == NULL) {
    goto fail; //Date
    }
    }
    ...
    if(invoice->payment_status != simplebilly_api_payment_status__NULL) {
    cJSON *payment_status_local_JSON = payment_status_convertToJSON(invoice->payment_status);
    ...
    if(invoice->payment_terms_text) {
    if(cJSON_AddStringToObject(item, "paymentTermsText", invoice->payment_terms_text) == NULL) {
    goto fail; //String
    }
    }


    // invoice->preceding_sales_voucher_id
    if(invoice->preceding_sales_voucher_id) {
    if(cJSON_AddStringToObject(item, "precedingSalesVoucherId", invoice->preceding_sales_voucher_id) == NULL) {
    goto fail; //String
    }
    }


    // invoice->preceding_sales_voucher_type
    if(invoice->preceding_sales_voucher_type) {
    cJSON *preceding_sales_voucher_type_local_JSON = preceding_sales_voucher_type_convertToJSON(invoice->preceding_sales_voucher_type);
    ...
    // invoice->receipt_confirmation_available
    if(invoice->receipt_confirmation_available) {
    if(cJSON_AddBoolToObject(item, "receiptConfirmationAvailable", *invoice->receipt_confirmation_available) == NULL) {
    goto fail; //Bool
    }
    }
    ...
    if(invoice->related_invoice_id) {
    if(cJSON_AddStringToObject(item, "relatedInvoiceId", invoice->related_invoice_id) == NULL) {
    goto fail; //String
    }
    }


    // invoice->relationship_type
    if(invoice->relationship_type) {
    if(cJSON_AddStringToObject(item, "relationshipType", invoice->relationship_type) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->sender_snapshot) {
    cJSON *sender_snapshot_local_JSON = _convertToJSON(invoice->sender_snapshot);
    ...
    // invoice->sent_at
    if(invoice->sent_at) {
    if(cJSON_AddStringToObject(item, "sentAt", invoice->sent_at) == NULL) {
    goto fail; //Date-Time
    }
    }
    ...
    if(invoice->service_period_end) {
    if(cJSON_AddStringToObject(item, "servicePeriodEnd", invoice->service_period_end) == NULL) {
    goto fail; //Date
    }
    }
    ...
    if(invoice->service_period_start) {
    if(cJSON_AddStringToObject(item, "servicePeriodStart", invoice->service_period_start) == NULL) {
    goto fail; //Date
    }
    }
    ...
    // invoice->status
    if (simplebilly_api_invoice_status__NULL == invoice->status) {
        goto fail;
    }
    cJSON *status_local_JSON = invoice_status_convertToJSON(invoice->status);
    ...
    if(cJSON_AddStringToObject(item, "subtotal", invoice->subtotal) == NULL) {
    goto fail; //String
    }
    ...
    // invoice->supplier_id
    if(invoice->supplier_id) {
    if(cJSON_AddStringToObject(item, "supplierId", invoice->supplier_id) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(invoice->tax_exemption_reason) {
    if(cJSON_AddStringToObject(item, "taxExemptionReason", invoice->tax_exemption_reason) == NULL) {
    goto fail; //String
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "totalAmount", invoice->total_amount) == NULL) {
    goto fail; //String
    }
    ...
    if(cJSON_AddStringToObject(item, "totalTax", invoice->total_tax) == NULL) {
    goto fail; //String
    }
    ...
    // invoice->vat_country
    if(invoice->vat_country) {
    cJSON *vat_country_local_JSON = country_code_convertToJSON(invoice->vat_country);
    ...
    // invoice->vat_special_case
    if(invoice->vat_special_case) {
    if(cJSON_AddStringToObject(item, "vatSpecialCase", invoice->vat_special_case) == NULL) {
    goto fail; //String
    }
    }
...
invoice_t *invoice_parseFromJSON(cJSON *invoiceJSON){
    ...
    // define the local variable for invoice->attachments
    ...
    // define the local variable for invoice->currency
    ...
    // define the local variable for invoice->discount_days
    int *discount_days_local_var = NULL;

    char *discount_percentage_local_str = NULL;

    // define the local variable for invoice->document_type
    simplebilly_api_document_type__e document_type_local_nonprim = 0;

    // define the local variable for invoice->dunning_level
    int *dunning_level_local_var = NULL;

    char *input_vat_amount_local_str = NULL;

    // define the local variable for invoice->input_vat_deductible
    ...
    // define the local variable for invoice->invoice_type
    simplebilly_api_invoice_type__e invoice_type_local_nonprim = 0;

    // define the local variable for invoice->is_cancelled
    int *is_cancelled_local_var = NULL;

    // define the local variable for invoice->is_draft
    int *is_draft_local_var = NULL;

    // define the local variable for invoice->is_eu_acquisition
    int *is_eu_acquisition_local_var = NULL;

    // define the local variable for invoice->is_eu_delivery
    int *is_eu_delivery_local_var = NULL;

    // define the local variable for invoice->is_intra_community_acquisition
    int *is_intra_community_acquisition_local_var = NULL;

    // define the local variable for invoice->is_reverse_charge
    ...
    // define the local variable for invoice->line_items
    _t *line_items_local_nonprim = NULL;

    // define the local variable for invoice->margin25a
    ...
    // define the local variable for invoice->payment_status
    ...
    // define the local variable for invoice->preceding_sales_voucher_type
    preceding_sales_voucher_type_t *preceding_sales_voucher_type_local_nonprim = NULL;

    // define the local variable for invoice->receipt_confirmation_available
    ...
    // define the local variable for invoice->sender_snapshot
    ...
    // define the local variable for invoice->status
    ...
    // define the local variable for invoice->vat_country
    country_code_t *vat_country_local_nonprim = NULL;

    char *vat_special_case_local_str = NULL;

    // invoice->attachments
    cJSON *attachments = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "attachments");
    ...
    // invoice->billing_period_end
    cJSON *billing_period_end = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "billingPeriodEnd");
    ...
    // invoice->billing_period_start
    cJSON *billing_period_start = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "billingPeriodStart");
    ...
    // invoice->cancellation_date
    cJSON *cancellation_date = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "cancellationDate");
    ...
    // invoice->cancellation_invoice_id
    cJSON *cancellation_invoice_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "cancellationInvoiceId");
    ...
    // invoice->cancellation_reason
    cJSON *cancellation_reason = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "cancellationReason");
    ...
    // invoice->contract_id
    cJSON *contract_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "contractId");
    ...
    // invoice->customer_id
    cJSON *customer_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "customerId");
    ...
    // invoice->discount_amount
    cJSON *discount_amount = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "discountAmount");
    ...
    // invoice->discount_days
    cJSON *discount_days = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "discountDays");
    ...
    *discount_days_local_var = discount_days->valuedouble;
    ...
    // invoice->discount_percentage
    cJSON *discount_percentage = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "discountPercentage");
    ...
    // invoice->document_type
    cJSON *document_type = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "documentType");
    ...
    // invoice->dunning_level
    cJSON *dunning_level = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "dunningLevel");
    ...
    *dunning_level_local_var = dunning_level->valuedouble;
    ...
    // invoice->input_vat_amount
    cJSON *input_vat_amount = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "inputVatAmount");
    ...
    // invoice->input_vat_deductible
    cJSON *input_vat_deductible = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "inputVatDeductible");
    ...
    *input_vat_deductible_local_var = input_vat_deductible->valueint;
    ...
    // invoice->input_vat_percentage
    cJSON *input_vat_percentage = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "inputVatPercentage");
    ...
    // invoice->introduction_text
    cJSON *introduction_text = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "introductionText");
    ...
    // invoice->invoice_type
    cJSON *invoice_type = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "invoiceType");
    ...
    // invoice->is_cancelled
    cJSON *is_cancelled = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isCancelled");
    ...
    *is_cancelled_local_var = is_cancelled->valueint;
    ...
    // invoice->is_draft
    cJSON *is_draft = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isDraft");
    ...
    *is_draft_local_var = is_draft->valueint;
    ...
    // invoice->is_eu_acquisition
    cJSON *is_eu_acquisition = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isEuAcquisition");
    ...
    *is_eu_acquisition_local_var = is_eu_acquisition->valueint;
    ...
    // invoice->is_eu_delivery
    cJSON *is_eu_delivery = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isEuDelivery");
    ...
    *is_eu_delivery_local_var = is_eu_delivery->valueint;
    ...
    // invoice->is_intra_community_acquisition
    cJSON *is_intra_community_acquisition = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isIntraCommunityAcquisition");
    ...
    *is_intra_community_acquisition_local_var = is_intra_community_acquisition->valueint;
    ...
    // invoice->is_reverse_charge
    cJSON *is_reverse_charge = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "isReverseCharge");
    ...
    *is_reverse_charge_local_var = is_reverse_charge->valueint;
    ...
    // invoice->issue_date
    cJSON *issue_date = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "issueDate");
    ...
    // invoice->ledger_account
    cJSON *ledger_account = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "ledgerAccount");
    ...
    // invoice->margin25a
    cJSON *margin25a = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "margin25a");
    ...
    *margin25a_local_var = margin25a->valueint;
    ...
    // invoice->margin25a_gross
    cJSON *margin25a_gross = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "margin25aGross");
    ...
    // invoice->margin25a_purchase_price
    cJSON *margin25a_purchase_price = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "margin25aPurchasePrice");
    ...
    // invoice->notes
    cJSON *notes = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "notes");
    ...
    // invoice->order_number
    cJSON *order_number = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "orderNumber");
    ...
    // invoice->original_pdf_path
    cJSON *original_pdf_path = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "originalPdfPath");
    ...
    // invoice->paid_amount
    cJSON *paid_amount = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "paidAmount");
    ...
    // invoice->payment_due_date
    cJSON *payment_due_date = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "paymentDueDate");
    ...
    // invoice->payment_status
    cJSON *payment_status = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "paymentStatus");
    ...
    // invoice->payment_terms_text
    cJSON *payment_terms_text = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "paymentTermsText");
    ...
    // invoice->preceding_sales_voucher_id
    cJSON *preceding_sales_voucher_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "precedingSalesVoucherId");
    ...
    // invoice->preceding_sales_voucher_type
    cJSON *preceding_sales_voucher_type = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "precedingSalesVoucherType");
    ...
    // invoice->receipt_confirmation_available
    cJSON *receipt_confirmation_available = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "receiptConfirmationAvailable");
    ...
    *receipt_confirmation_available_local_var = receipt_confirmation_available->valueint;
    ...
    // invoice->related_invoice_id
    cJSON *related_invoice_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "relatedInvoiceId");
    ...
    // invoice->relationship_type
    cJSON *relationship_type = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "relationshipType");
    ...
    // invoice->sender_snapshot
    cJSON *sender_snapshot = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "senderSnapshot");
    ...
    // invoice->sent_at
    cJSON *sent_at = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "sentAt");
    ...
    // invoice->service_period_end
    cJSON *service_period_end = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "servicePeriodEnd");
    ...
    // invoice->service_period_start
    cJSON *service_period_start = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "servicePeriodStart");
    ...
    // invoice->status
    cJSON *status = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "status");
    ...
    // invoice->subtotal
    cJSON *subtotal = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "subtotal");
    ...
    // invoice->supplier_id
    cJSON *supplier_id = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "supplierId");
    ...
    // invoice->tax_exemption_reason
    cJSON *tax_exemption_reason = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "taxExemptionReason");
    ...
    // invoice->total_amount
    cJSON *total_amount = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "totalAmount");
    ...
    // invoice->total_tax
    cJSON *total_tax = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "totalTax");
    ...
    // invoice->vat_country
    cJSON *vat_country = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "vatCountry");
    ...
    // invoice->vat_special_case
    cJSON *vat_special_case = cJSON_GetObjectItemCaseSensitive(invoiceJSON, "vatSpecialCase");
